...
class BlogManager {
    ...
    openBlogPost(postId) {
        ...
        // Create or show blog reader
        let blogReader = document.querySelector('.blog-reader');
        if (!blogReader) {
            blogReader = document.createElement('div');
            blogReader.className = 'blog-reader';
            blogGrid.parentNode.insertBefore(blogReader, blogGrid);
        }
        
        blogReader.style.display = 'block';
        blogReader.innerHTML = this.renderBlogPost(post);
